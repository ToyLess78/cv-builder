...
interface IMonthYearPickerWithRangeProps {
	...
	setDuration: React.Dispatch<
		React.SetStateAction<Nullable<Date | (Date | null)[]>>
	>;
	children?: ReactNode;
	isYear: boolean;
	isPresent?: boolean;
...
export const MonthYearPickerWithRange: FC<IMonthYearPickerWithRangeProps> = ({
	...
	const handleChangeRangeDates = (
		e: FormEvent<(Date | null)[], React.SyntheticEvent<Element, Event>>,
	) => {
		setRangeDates(e.value);
	};
	const handleChangeSingleDate = (
			...
			{isYear && isPresent && (
				<>
					<Calendar
						...
						required
					/>
					<label htmlFor="single-year-calendar" className={styles.label}>
						Start Year & Present
					</label>
				</>
			)}
			{!isYear && isPresent && (
				<>
					<Calendar
						...
						required
					/>
					<label htmlFor="single-month-calendar" className={styles.label}>
						Start Date & Present
					</label>
				</>
			)}
			{isYear && !isPresent && (
				<>
					<Calendar
						...
						required
					/>
					<label htmlFor="range-year-calendar" className={styles.label}>
						Start & End Year
					</label>
				</>
			)}
			{!isYear && !isPresent && (
				<>
					<Calendar
						...
						required
					/>
					<label htmlFor="range-month-calendar" className={styles.label}>
						Start & End Date
					</label>
				</>
			)}
			...
			{isYear && (
				<>
					<Calendar
						...
						required
					/>
					<label htmlFor="single-year" className={styles.label}>
						Year
					</label>
				</>
			)}
			{!isYear && (
				<>
					<Calendar
						...
						required
					/>
					<label htmlFor="single-month" className={styles.label}>
						Month & Year
					</label>
				</>
			)}
