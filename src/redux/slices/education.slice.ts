...
const initialState: EducationState = loadFromLocalStorage(
	RootConstants.Education,
) || {
	...
	data: [
		{
			...
			location: "California",
			description:
				"<ul><li>Completed Master of Computer Science degree from Pepperdine University in California.</li></ul>",
		},
		{
			id: "30T210000sddZ",
...
const educationSlice = createSlice({
	...
	reducers: {
		...
		setEditedEducation(
			state,
			action: PayloadAction<{ education: Partial<IEducation> }>,
		) {
			const { education } = action.payload;

			const existingEducationIndex = state.data.findIndex(
