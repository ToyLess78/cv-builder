...
const initialState: IAsideState = loadFromLocalStorage(
	RootConstants.Skills,
) || {
...
const skillsSlice = createSlice({
	...
	reducers: {
		...
		setSkills(
			state,
			action: PayloadAction<Partial<IAsideState[RootConstants.Skills]>>,
		) {
			state.skills = { ...state.skills, ...action.payload };
		},
		setAdditional(
			state,
			action: PayloadAction<Partial<IAsideState[RootConstants.Additional]>>,
		) {
