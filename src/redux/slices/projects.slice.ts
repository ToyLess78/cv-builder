...
const initialState: IProjectsState = loadFromLocalStorage(
	RootConstants.Projects,
) || {
	...
	data: [
		{
			...
			description:
				"<p>A platform for recording, storing, and supporting podcasts. The solution allows you to record a podcast in real time (people can go in and comment on the podcast while recording). Users can create favorite podcast lists, leave comments and likes. A private podcast option is also available.</p>",
		},
		{
...
const projectsSlice = createSlice({
	...
	reducers: {
		...
		setEditedProject(
			state,
			action: PayloadAction<{ project: Partial<IProject> }>,
		) {
			const { project } = action.payload;

			const existingProjectIndex = state.data.findIndex(
